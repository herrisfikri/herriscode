        ...
        if ($row = mysqli_fetch_assoc($resultl)) {
            ...
            if ($row['add_card'] == 0) {
                ...
                if (!empty($Uname) && !empty($Email)) {
                    //check if there any user had already the email
                    $sql = "SELECT email FROM users WHERE email=? AND id NOT like ?";
                    $result = mysqli_stmt_init($conn);
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        echo "SQL_Error";
                        exit();
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            $sql="UPDATE users SET username=?, grade=?, gender=?, email=?, user_date=CURDATE(), device_uid=?, device_dep=?, add_card=1 WHERE id=?";
                            $result = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                ...
                                mysqli_stmt_bind_param($result, "sdssssi", $Uname, $Grade, $Gender, $Email, $dev_uid, $dev_name, $user_id );
                                mysqli_stmt_execute($result);
            ...
            if ($row['add_card'] == 0) {
                ...
                if (empty($Uname) && empty($Email)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        mysqli_stmt_bind_param($result, "di", $Grade, $user_id);
                        mysqli_stmt_execute($result);
                        $resultl = mysqli_stmt_get_result($result);
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            $sql = "SELECT device_dep FROM devices WHERE device_uid=?";
                            ...
                            if (!empty($Uname) && !empty($Email)) {
                                ...
                                $sql="UPDATE users SET username=?, grade=?, gender=?, email=?, device_uid=?, device_dep=? WHERE id=?";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                            ...
                            echo "The grade is already taken!";
                            exit();
                        }
                    }
                }
